...
class InitialFragment : Fragment(),View.OnClickListener {
    ...
    private fun validation(): Boolean {
        if(emailTextInputEditText?.text==null|| emailTextInputEditText?.text?.toString()?.trim()?.isEmpty()!!)
        {
        ...
        }else
        {
            ...
            {
                if(gmailUser.username.toString().equals(emailTextInputEditText.text.toString().trim(),true))
                {
            ...
            Utils.showErrorWithTextInput(activity,emailTextInputLayout,R.drawable.ic_error,"Couldn't find your Google Account")
            return false;
        }
        return true;
    }
